...
namespace Logic.Models.XmlSerializers
{
    internal class BaseXmlSerializer
    {
        ...
        {
            return string.IsNullOrEmpty(customRoot)
                ? new XmlSerializer(type)
                : new XmlSerializer(type, new XmlRootAttribute(customRoot));
        }
        ...
        {
            var xml = StringSerialize(project);
        ...
        private TextWriter WriterSerialize<T>(T project)
        {
            ...
            if (_s == null) _s = CreateXmlSerializer(_customRoot, project.GetType());
